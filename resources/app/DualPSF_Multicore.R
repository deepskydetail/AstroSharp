...
DualPSFMC <- function(NNModeltemp,
                          PSFval,
                          PSFval2,
                          Clr,
                          Preview,
  ...
  withProgress(message = "Creating Star Mask...", detail = "Hope it works!", 
               value = 0.10,
               {
                 ...
                 sensi1 <- as.numeric(sensi)
                 thresh1 = as.numeric(thresh)
                 mix.1 = as.numeric(mix)
                 mix2.1 = as.numeric(mix2)
                 StarNoise1 = as.numeric(StarNoise)
    
                 starmaskfun <- function(tif1, sensi = sensi1, thresh = thresh1, 
                                         mix = mix.1, mix2 = mix2.1, StarNoise = StarNoise1){
                 ...
                 #for(i in 2:length(colseq)){
                   ...
                   for(j in 2:length(rowseq)){
                     ...
                     if((colseq[i-1]) == 1 & (rowseq[j-1])==1){
                      ...
                      data.frame(predvals, colvals, rowvals)#, btwovals) 
